...
export default {
  ...
  created() {
    ...
    console.log('screens length', this.$slots.default.filter((c) => !!c.componentOptions).length);

    // Watch for changes to this.$magpie.currentScreenIndex
    this.$watch(
      () => this.$magpie.currentScreenIndex, // The property to watch
      (newValue, oldValue) => {
        console.log(`Screen index changed from ${oldValue} to ${newValue}`);
        // Add any additional logic you want to execute when the screen index changes
      }
    );
    ...
    // Preload assets (images, audio, video)
